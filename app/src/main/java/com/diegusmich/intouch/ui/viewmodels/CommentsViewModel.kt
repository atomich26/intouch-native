...
class CommentsViewModel : StateViewModel() {
    ...
    private var onAddNewCommentJob: Job? = null
    ...
    fun onUpdateCommentText(input: String) {
        _commentText.value = _commentText.value?.copy(
            inputValue = input,
            isValid = input.isNotBlank() && input.length < 250
        )
    }

    fun onAddNewComment() {
        if (onAddNewCommentJob?.isActive == true)
            return

        onAddNewCommentJob = viewModelScope.launch {
                ...
                try {
                    postId?.let { postId ->
                        ...
                        PostRepository.addComment(
                            postId,
                            commentText.value?.inputValue.toString()
                        )
                        onUpdateCommentText("")
